...
The ledger inventory totals twelve developmental runs and 89 events: 86 completed, one failed and two reserved. Known usage totals 227,848 input and 4,222 output tokens; three events lack usage. These counts describe adapter records, not successful physical actions or independently verified charges. The complete table and per-attempt evidence are in [RESULTS.md](../RESULTS.md).

The observer now displays resident quantities and exports action-transition reports. Future Gemini calls request a brief stated intent, recorded separately from the action. No such statements exist for the historical trials above. This instrument changes the response contract and therefore requires a new recorded condition. A generated explanation must not be treated as a faithful account of hidden computation or as evidence that an intended action occurred.
