...
## Photo floors, authored entrance and connected review

The user's approved home contained a valid floor texture. Its derivative was exactly coplanar with the solid floor. New horizontal photo geometry is inset 6mm into the room, and the runtime/viewer also repair legacy coplanar derivatives in memory. Original uploads and saved user records are unchanged. A read-only copy of the actual approved GLB now visibly renders its wood floor with logarithmic depth enabled.

The walker now spawns beside the marked ground-floor entrance, facing into that room. The plan exposes Set home entrance; selecting another exterior room wall moves the entry designation and draws an Entrance marker. Floor and ceiling photo buttons open those surfaces directly. Review previews have overview and inside views, including ceilings.

My contributions and Review improvements now live in Account Center sections with browser Back support. The same protected review component opens inside the world from My contributions. Legacy capture moderation links lead to the account review section; advanced system administration remains separately accessible. Review still uses the existing authorization and exact submitted-revision endpoint, requires a loaded preview before approval and a note for changes requested.

Local verification: 24 focused geometry/layout/photo/authority tests; desktop and mobile editor acceptance including moving the entrance, wall/floor/ceiling placement, saving, submission, derivative generation, reopening and recovery; shared review acceptance at 1100/412px with real generated GLB, exact-version decisions, private status, failed-preview blocking and access denial; actual E-key runtime using a read-only copy of the user's approved layout and GLB, marked-door spawn/yaw, visible floor, exit restoration and private denial. Source graph and diff checks passed. The skill browser smoke displayed the account section; localhost reCAPTCHA rejects automation as expected, so authenticated acceptance uses bounded staging attestation rather than bypassing authorization. Staging deployment and acceptance are recorded in the follow-up run outputs.

Deployment acceptance caught an account entry packaging omission: the raw account module imported game source files excluded from hosting. The account workspace is now an explicit shared runtime bundle entry, its HTML reference is rewritten, and artifact verification requires that entry. Review inside view also faces inward and exposes floor/ceiling inspection buttons. Desktop/mobile generated ceiling screenshots were inspected.

Final deployed acceptance: staging build `5.2.0+d73394916971.56b6bfc3840ee72e.staging`, plus staging `submitRealityCaptureHybrid` with horizontal photo insets. The deployed Account Center approved only a disposable fixture, My contributions and browser Back worked, and an independent account rendered the public exterior. Replacement and deletion/withdrawal checks passed. Live private-home acceptance passed with three wall/floor/ceiling placements, upload, CPU derivative, save/reopen, continuation preserving all three, privacy/cost gates and fixture deletion. The final account browser smoke has no missing module requests; headless third-party reCAPTCHA errors remain a test-environment limitation, with authenticated checks using disposable staging attestation. No production changes, real-user approvals or physical Android testing.
