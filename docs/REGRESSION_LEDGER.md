...
# 2026-08-15 — Structure heuristics delete bridge bodies/supports and tunnel exits collapse the camera

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: curved ramps and dense interchanges could become thin floating road ribbons; bridge ends changed abruptly from ground to open slab; support columns could pass through a lower carriageway; tunnel tags could select enclosed camera behavior without a buried shell; and a short tunnel exit could snap the chase camera inside/below the car.
- Root cause: the renderer independently classified `clutteredInterchange` and suppressed every road body/support for short, curved, ramp-like, or dense features. Exact vehicle roads also published two overlapping body implementations with different thickness/end rules, and abutments were connector-only. Support stations described only a center point, not a road-aware column layout. Tunnel camera mode read semantic tags/current nearest-road identity instead of compiled shell occupancy, and it released when the actor crossed a portal even while the desired trailing camera was still inside. The main journey gate used a fixed three-second host clock and ignored its requested installed-Chrome channel.
- Resolution: one `compiled_transport_structure_assembly` owns elevated body coverage, surface samples, tapered thickness, closed ends, abutments, support stations, and actual column layouts. Exact structures retain engineered detail. Complete generalized bridges receive sparse non-colliding visual supports only. Candidate columns are tested against the entire vertical interval of nearby driveable roads and can move outside the deck before a station is withheld. One compiled spline-tunnel system owns buried ranges, closed shell, graph portals, local portal apertures, wall collision, and camera envelope; no Boolean terrain carving or separate tunnel scene exists. Dedicated tunnel camera envelope/controller modules retain containment until both actor and trailing camera clear a compiled portal zone. Obsolete overlapping vehicle deck boxes and blanket clutter suppression were removed.
- Evidence: pure fixtures require one vehicle body, curved-ramp supports, crossing-excluded stations, outside-column relocation above a lower road, generalized visual supports, exposed-tag camera rejection, portal-transition retention, and shell-exit controller release. Installed Chrome passed Holland short-underpass (25.47 m, zero collision, 6.09 m minimum camera distance), Pregerson exact ramp merge (57.75 m, one body, 40 support columns, one abutment), Golden Gate/Bay bridge driving (75.24 m, stable camera), Monaco (115 exact tunnels, 1,095 buried roof samples, zero tunnel-center/street false blocks), and the London/San Francisco regional matrix (845/585 bridges, 2,322/168 tunnels, zero unowned terrain cells). Gameplay/aerial frames were inspected and consoles were clean.
- Guard: `npm run test:phase3-structures`, `npm run test:tunnel-system`, `npm run test:transport-surface`, `npm run test:monaco-tunnels-browser`, `PHASE3_JOURNEY_IDS=holland-short-underpass WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`, `PHASE3_JOURNEY_LOCATIONS=pregerson-interchange WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`, `PHASE3_JOURNEY_LOCATIONS=golden-gate WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`, `npm run test:regional-structures-browser`, `npm run test:bridge-road-conflict-index`, `npm run test:road-publication-ownership`, `npm run test:module-versions`, and `npm run test:maintainability`.
- Never reintroduce: renderer-local blanket support/body suppression, overlapping vehicle deck-body owners, supports placed without lower-road vertical-corridor checks, generalized hard support/barrier/tunnel collision, tag-only tunnel camera mode, camera release based only on actor position, portal masks along an entire buried route, runtime Boolean ground carving, a separate tunnel scene, a second tunnel shell owner, or fixed host-clock journey assertions as proof of a physical transition.

# 2026-08-15 — Elevated graph joins leave abrupt or missing ramp transitions

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: ramps and bridge approaches could visibly or physically step at the road they joined, making the car bump or jump even though both features belonged to the same transport graph node.
- Root cause: graph-node endpoint constraints were deliberately marked with `endpoint: null` and a fixed two-metre span. The compiler wrote the exact target into one sample, then the generic lower-bounded bridge smoother lifted that sample again. A controlled motorway-link merge retained a 14 cm vertical step.
- Resolution: mapped graph endpoints now participate in the existing endpoint transition gate, and their reserved blend span follows elevation change and maximum road grade. A constrained elevated-profile pass treats feasible graph-node heights as exact and reconciles neighboring samples inside a two-sided grade cone. Mandatory crossing and absolute clearance bounds still reject physically impossible graph targets. Tunnel/subgrade reconciliation remains on its previous code path.
- Evidence: the controlled merge now has an exact endpoint, maximum grade `0.1000004`, and complete visible-body coverage. Installed Chrome crossed OSM way `120061089` at the Pregerson Interchange with a `0.0000 m` graph-node step, full deck-body coverage, `0.030 m` maximum surface error, zero collisions, and zero frames over 50 ms. The Holland short-underpass retained its compiled shell exit, zero center collision, zero camera occlusion, `0.089 m` maximum surface error, and zero frames over 50 ms; screenshots were inspected.
- Guard: `npm run test:phase3-structures`, `npm run test:transport-surface`, `npm run test:tunnel-system`, `npm run test:monaco-structure-ownership`, `PHASE3_JOURNEY_LOCATIONS=pregerson-interchange WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`, `PHASE3_JOURNEY_IDS=holland-short-underpass WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`, and `npm run test:module-versions`.
- Never reintroduce: non-endpoint graph anchors at mapped endpoints, fixed two-metre ramp blends, post-constraint smoothing that moves a feasible shared node, forcing an impossible node below mandatory bridge clearance, separate visual/body elevation ownership, or applying the elevated exact-join solver to tunnels.

# 2026-08-15 — Covered roads fabricated a second tunnel system and branch walls crossed lanes

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: around `43.7274, 7.4134` in Monaco, large gray roof, wall, and portal slabs stood above ground. Closely connected passages overlapped, and walls continued across splitting lanes, producing tangled intersections and blocked driving space.
- Root cause: `covered=yes` and `tunnel=building_passage` independently activated a legacy visual-box builder, synthetic side-wall collision, and tag-driven tunnel camera mode. The compiled terrain-buried spline-tunnel system remained active separately. Because each covered way built its own enclosure without graph junction ownership, the app had two competing tunnel authorities and no branch chamber.
- Resolution: covered/building-passage tags no longer publish tunnel geometry, collision, portals, or enclosed-camera state. Their mapped building or structure owns cover and the road retains its normal drive surface. Only a terrain-validated true-tunnel model may publish a shell. Compatible true tunnels meeting at a graph branch compile width-derived junction zones; rendering removes independent side walls/shoulders there while retaining crown cover, and collision subtracts the identical zones. Nearby unconnected bores remain independent.
- Evidence: the installed-Chrome Monaco source run retained nine nearby covered roads while publishing zero nearby structure instances. A ground probe on `Liaison Marquet` (`osm:way:503475655`) reported no synthetic transport collision and `cameraInside=false` with `no_compiled_shell`. The broader Monaco run sampled 428 true-tunnel stations with every roof below terrain, 80/80 wall hits, zero center/street false blocks, and no console errors. A real-input Holland shell journey exited cleanly with zero center collision or camera occlusion. The synthetic three-way graph fixture compiles one shared junction zone on every branch and proves collision ends before the chamber.
- Guard: `npm run test:monaco-structure-ownership`, `npm run test:monaco-tunnels-browser`, `npm run test:tunnel-system`, `npm run test:phase3-structures`, `npm run test:transport-surface`, and `PHASE3_JOURNEY_IDS=holland-short-underpass WE3D_BROWSER_CHANNEL=chrome npm run test:phase3-structure-journeys`.
- Never reintroduce: covered-tag freestanding boxes, covered-tag wall collision, covered-tag camera mode, renderer-owned branch decisions, per-way walls through a shared graph junction, proximity-only merging of nearby tunnels, or a second tunnel mesh/collider owner.

# 2026-08-15 — Dense fixed worlds retain gigabytes and tunnel evidence shows the portal/outside

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: a New York game tab could retain roughly 1.5 GB, travel showed periodic stalls, tunnel portals contained multi-metre vertical steps, and screenshots presented a short underpass or outside portal as if it proved a true enclosed tunnel.
- Root cause: decoded Shortbread tiles and raw Overpass responses survived after the same content was compiled into the playable world. The fixed 14-kilometre outer context also requested zoom-14 local street detail already owned by the lossless exact core. The subgrade smoother reapplied terrain-cover bounds after endpoint ties, leaving a 7.07 m mismatch at a real graph join. Finally, the journey selected the longest regional tunnel and treated `null` as a numeric zero when positioning its “interior” proof camera.
- Resolution: release provider staging caches after compilation, retain persistent reload caches, and load the unchanged fixed regional radius at zoom 13 while keeping the exact core lossless. Reconcile tunnel portals through a mirrored grade-constrained solver that relaxes only the already-tapered mapped portal zone. Report the selected source name and separate true mid-shell visual proof from the exit traversal. Tunnel shells use differentiated lining tones and regularly spaced paired emissive strips in one instanced batch.
- Evidence: installed Chrome reduced post-GC JavaScript heap from 1,856,833,497 bytes to 644-712 MB with zero decoded Shortbread/raw Overpass entries retained. A real-input New York tunnel traversal held the chase camera 2.12-2.33 m above its compiled road, reduced the graph join discrepancy to 0.10 m, recorded p95 17.7-18.6 ms frames with zero 100 ms stalls, and captured a deep Brooklyn-Battery Tunnel interior with no visible outside city or sky.
- Guard: `npm run test:fixed-regional-context`, `npm run test:transport-surface`, `npm run test:tunnel-system`, `npm run test:phase3-structures`, `npm run test:module-versions`, and `PHASE3_JOURNEY_IDS=holland-tunnel PHASE3_USE_INSTALLED_CHROME=1 node scripts/test-phase3-structure-journeys.mjs`. The browser journey enforces an 850 MB JavaScript heap ceiling and zero retained provider staging caches.
- Never reintroduce: actor-driven world streaming, shrinking the authoritative fixed-world boundary, retaining decoded/raw provider staging after publication, duplicating the exact street core at outer-context resolution, cover bounds that reopen a graph-node step after portal reconciliation, unlabeled regional tunnel substitution, `null` numeric sentinels in journey placement, or portal/outside screenshots presented as tunnel-interior proof.

# 2026-08-15 — Night gameplay becomes unreadable and DeFlock obscures its objectives

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: Earth gameplay could fade to almost-black after a good initial load, the DeFlock HUD and navigation blocked too much of the playfield, mapped cameras were difficult to identify at their minimap positions, and car/plane chase cameras felt jumpy.
- Root cause: astronomical night and weather multipliers could combine below a playable illumination floor. DeFlock reused full-size persistent panels and its exact camera meshes had no distant visual landmark. The chase camera used frame-dependent position damping, updated its collision arm in raycast-sized chunks, and the plane look target was not smoothed.
- Resolution: the existing live astronomical cycle retains day, sunset, sunrise, and night while enforcing playable hemisphere, fill, ambient, exposure, and fog values. DeFlock uses a compact right-anchored HUD/navigation layout and every exact mapped camera adds one depth-visible state-colored ring, beam, and elevated beacon. Car and plane chase rigs use a shared time-based exponential damping policy; collision-arm and plane look-target transitions are smoothed without adding another controller or render-interpolation system.
- Evidence: installed Chrome passed desktop and iPhone DeFlock journeys, dedicated wide-street day/night captures, live Baltimore loading and disabling of one of 24 mapped OSM cameras, and 75-frame driving/flight samples. Night retained visible roads, buildings, terrain, and player. Motion p95 camera-offset steps were 0.051 m driving and 0.222 m flying, with zero fatal application errors.
- Guard: `npm run test:deflock-browser`, `npm run smoke:deflock-live`, `npm run test:deflock-model`, `npm run test:phase5-controls`, `npm run test:css`, and `npm run test:module-versions`.
- Never reintroduce: near-black astronomical/weather multiplication, a centered full-width hunt panel, overlapping mobile HUD/minimap bounds, exact camera meshes without a distant marker, fixed-per-frame chase damping, chunk-snapped collision arms, an unsmoothed plane look target, or a second vehicle camera controller.

# 2026-08-15 — DeFlock cameras stand in traffic, ignore mounts, and cannot start in-world

- Status: resolved locally on `steven/earth-core-recovery`; not pushed or deployed. User Chrome acceptance remains open.
- Symptom: camera/pole models clustered in road lanes, overhead data rendered as ground poles, virtual disables changed only color instead of falling, and DeFlock could be selected only before entering the world.
- Root cause: source normalization discarded `camera:mount` and height tags, every source used one three-metre pole at the raw point, disable state updated only instance colors, and the current-world Games menu had no built-in DeFlock transition.
- Resolution: exact OSM coordinates remain source authority. Mapped height and mount semantics select pole versus traffic-signal/gantry/bridge/wire presentation. Non-overhead points inside a nearby mapped road footprint receive a deterministic curb-side visual anchor; explicit overhead mounts remain at the mapped point with a cross-road support. A newly disabled ground pole tips from its base and an overhead camera drops while its support remains. The first in-world Games action starts the registered DeFlock plugin against the existing Earth scene and roads.
- Evidence: installed Chrome retained the exact source transform, verified pole clearance or an already-off-road point, rendered a 4.2 m mapped pole and overhead traffic-signal support, observed one active fall and a settled camera 0.32 m above ground, and restarted DeFlock in-world without changing the scene UUID or 13,688 loaded roads. The live Baltimore smoke loaded 24 OSM cameras: 8 received curb adjustment, 6 retained explicit overhead mounts (5 traffic signals and 1 bridge), and the disabled pole visibly settled on the ground. Zero fatal errors were recorded.
- Guard: `npm run test:deflock-model`, `npm run test:deflock-browser`, `npm run smoke:deflock-live`, `npm run test:module-versions`, and `npm run test:css`.
- Never reintroduce: discarding mount/height tags, moving explicit overhead mounts to a guessed curb, leaving ordinary virtual poles in a driveable surface, changing the stored OSM coordinate to achieve visual clearance, color-only disable feedback, dropping the mapped support with an overhead camera body, reloading the current Earth world to start DeFlock, or hiding the mode outside the title screen.
