...
## 2026-08-20 audit checkpoint 10 — far context is admitted before provider fetch

### First authoritative loss

- The complete Manhattan world did not show a retained lifecycle leak: forced
  GC after walking changed used heap by only 1.09 MB with no movement network
  requests, Main Menu released the world layers and reduced used heap to 38.29
  MB, and reload heap was lower than the first load. Measurements are from the
  local Chrome run and are not claimed as hardware-independent limits.
- The expensive loss occurred earlier. Far mapped context requested all 256
  available z14 building tiles for the 28 km square before selection budgets.
  It decoded 766,918 footprints, retained 170,748, and transferred 23.75 MB of
  Shortbread vector data. Geometry rejection could not recover network,
  decoding, or transient allocation already spent.

### Completed bounded authority repair

- Far-context tile admission now owns the resource boundary before fetch. It
  keeps the nearest half of the 160-tile limit contiguous and uses
  deterministic farthest-point sampling for the remainder so context reaches
  every outer direction rather than becoming a city-shaped crop.
- The far exact-building budget is 6,000 and the total far instance budget is
  120,000. The building provider remains at z14 because the mapped building
  layer is unavailable at the lower context zoom. Source identities,
  provenance, footprint deduplication, near buildings, and one final far
  renderer remain intact.
- Final diagnostics publish the authority, available/requested/skipped tile
  counts, and both geometry budgets. The worldwide assembled gate rejects a
  world that fetches beyond the budget or publishes beyond either building
  limit. A 20x20 fixture requires exactly 160 unique admitted tiles and
  preservation of all four outer extents.

### Verified effect and remaining blockers

- On the same Manhattan path, the change requested 160/256 building tiles,
  transferred 17.31 MB of vector data, published 80,241 far buildings, reduced
  backing-store memory by about 8.47 MB, and reduced triangles from about 4.32
  million to 3.13 million. The assembled skyline remained tall and visible;
  near-world counts remained 10,763 buildings and 9,037 roads.
- Source verification, focused Manhattan assembly, source and packaged
  actor/vehicle verification, hosting artifact verification, and the new
  worldwide resource contract passed. The local dirty-source artifact was
  `4.3.0+88c3ff8b88a7.fa948a89c08ff98e.staging`; it was never deployed.
- The complete packaged matrix remained red and exposed provider-sensitive
  exact topology loss at Golden Gate (one/6.0511 m), Monaco (two/8.9704 m plus
  47.727% grade), Manhattan (nine/9.4423 m), and Tokyo (twelve/15.9661 m plus
  14.646% grade). London, Monaco, Tokyo, rural Iowa, and parts of Manhattan are
  also visibly incoherent even when numeric layer checks are green.

Next bounded task: trace exact source-node identity and provider selection into
deduplication and vertical topology, using the failing Golden Gate/Monaco/
Manhattan/Tokyo records. Repair the first shared authoritative loss; do not add
city patches, weaken the matrix, or conflate this with the facade-frontage
audit. Do not push or deploy.
