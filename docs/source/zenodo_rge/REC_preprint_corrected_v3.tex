...
We establish four fundamental theorems: (A) the \emph{Entropic Ceiling} $\Sbar(d) \leq 1$, (B*) the \emph{Corrected Growth Ratio Bound} $\rtilde(d) \leq 1 + 1/S(d)$ (for $S(d)>0$), (C*) \emph{Asymptotic Convergence} $\rtilde(d) \to 1$ on depths where the ratio is defined, and (D) the \emph{Geometric Bound} $\zeta = \dH/D \leq 1$ for volume-convergent recursive geometries. We also document that the earlier global claim $\rtilde(d)\le2$ is false without additional assumptions.
...
\begin{enumerate}[nosep]
    ...
    \item[(ii)] The growth factor $\rtilde(d) = S(d+1)/S(d)$ (on depths with $S(d)>0$)?
...
\begin{enumerate}[label=(C\arabic*), leftmargin=*]
    ...
    \item Proof of the Entropic Ceiling $\Sbar(d)\le 1$ for all recursive 
          binary partitions.
    \item Correction of the growth theorem: for binary recursion and $S(d)>0$,
          $\rtilde(d)\le 1 + 1/S(d)$ (sharp).
    \item Proof that entropy growth converges asymptotically on defined depths:
...
\begin{theorem}[Corrected Growth Ratio Bound --- Theorem B*]
\label{thm:B}
For any probability distribution on a binary recursive partition and any depth
$d$ with $S(d)>0$:
\begin{equation}
    \rtilde(d) := \frac{S(d+1)}{S(d)} \leq 1 + \frac{1}{S(d)}
\end{equation}
In particular, if $S(d)\ge1$, then $\rtilde(d)\le2$.
\end{theorem}

\begin{theorem}[Asymptotic Convergence on Defined Depths --- Theorem C*]
\label{thm:C}
...
B* (Corrected Growth) & $\rtilde(d) \leq 1 + 1/S(d)$ & $S(d)>0$ & Attained when $\Delta(d+1)=1$ \\
C* (Asymptotic) & $\rtilde(d) \to 1$ & Ratio defined eventually & $d \to \infty$ \\
D (Geometric) & $\zeta \leq 1$ & Volume-convergent & Space-filling \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Status Labels (v3)}
\begin{table}[H]
\centering
\caption{Claim status in this corrected draft}
\label{tab:status_labels}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Item} & \textbf{Status} & \textbf{Note} \\
\midrule
Theorem A & Proven & Standard entropy maximization over $2^d$ outcomes \\
Theorem B* & Proven & Sharp bound from one-step conditional entropy increment \\
Theorem C* & Proven & Uses ratio identity $\rtilde(d)=1+\Delta(d+1)/S(d)$ \\
Theorem D & Proven & Equivalent to $Ns^D\le1$ after logarithmic rearrangement \\
Legacy claim $\rtilde(d)\le2$ (global) & Disproven & Counterexample: depth 1 split $p=1/3$ gives ratio $2.0889$ \\
5:4 resonance & Conjecture & Not part of proven information-entropy theorems \\
\bottomrule
\end{tabular}
\end{table}
...
$\rtilde(d)$ & Growth factor $S(d+1)/S(d)$ & Defined when $S(d)>0$; then $\rtilde(d)\in[1,\infty)$ \\
...
\subsection{Proof of Theorem B* (Corrected Growth Ratio Bound)}

\begin{proof}[Proof of Theorem \ref{thm:B}]
Define $\Delta(d+1):=S(d+1)-S(d)$.

\textbf{Step 1: One-step increment bound.}
By entropy chain-rule decomposition across one recursive split level with
branching factor $b=2$,
\begin{equation}
    S(d+1)=S(d)+\sum_i p_i(d)\,H(Q_i),
\end{equation}
where $Q_i$ is the conditional split distribution inside depth-$d$ cell $i$.
Since $H(Q_i)\le1$ for binary outcomes:
\begin{equation}
    0 \le \Delta(d+1)\le1.
\end{equation}

\textbf{Step 2: Ratio identity and bound.}
For depths where $S(d)>0$:
\begin{equation}
    \rtilde(d)=\frac{S(d+1)}{S(d)}=\frac{S(d)+\Delta(d+1)}{S(d)}
    =1+\frac{\Delta(d+1)}{S(d)}\le 1+\frac{1}{S(d)}.
\end{equation}

\textbf{Step 3: Corollary for the value 2.}
If additionally $S(d)\ge1$, then
\begin{equation}
    \rtilde(d)\le1+\frac{1}{S(d)}\le2.
\end{equation}
\end{proof}
...
We assume $S(d)>0$ for all sufficiently large $d$, so $\rtilde(d)$ is defined.

\textbf{Step 1: Ratio identity.}
From $\Delta(d+1):=S(d+1)-S(d)$:
\begin{equation}
    \rtilde(d)=1+\frac{\Delta(d+1)}{S(d)},
\end{equation}
and by the binary increment bound, $0\le\Delta(d+1)\le1$.

\textbf{Step 2: Case analysis.}
\emph{Case A:} $S(d)\to\infty$. Then $0\le \Delta(d+1)/S(d)\le 1/S(d)\to0$, so
$\rtilde(d)\to1$.

\emph{Case B:} $S(d)$ is bounded above. Since $S(d+1)\ge S(d)$ and $S(d)\ge0$, the
sequence $\{S(d)\}$ is monotone increasing and bounded, hence converges to some finite
$L\ge0$. Because the ratio is eventually defined, $L>0$. Therefore
$\Delta(d+1)=S(d+1)-S(d)\to0$, and so $\Delta(d+1)/S(d)\to0$.
Hence $\rtilde(d)\to1$.

\textbf{Conclusion.}
In all cases where $\rtilde(d)$ is eventually defined, $\lim_{d\to\infty}\rtilde(d)=1$.
\end{proof}
...
% Dynamic envelope (illustrative)
\addplot[black, thick, dashed, domain=1:20, samples=20] {1 + 1/x};
\addlegendentry{Envelope: $1 + 1/S(d)$ (using $S(d)\approx d$)}
...
\end{axis}
\end{tikzpicture}
\caption{Growth factor $\rtilde(d)$ versus depth. The ratio converges to 1 (Theorem C*) and is controlled by the corrected bound in Theorem B*.}
...
\toprule
\textbf{Distribution} & $\max \Sbar(d)$ & $\max \rtilde(d)$ & $\rtilde(d \to \infty)$ & $\Sbar \leq 1$? & B* passes? & Legacy $\rtilde \leq 2$? \\
\midrule
Uniform & 1.000 & 2.000 & 1.000 & \checkmark & \checkmark & \checkmark \\
Power-law ($\alpha = 1.5$) & 0.829 & 1.878 & 1.002 & \checkmark & \checkmark & \checkmark \\
Power-law ($\alpha = 2.0$) & 0.722 & 1.778 & 1.000 & \checkmark & \checkmark & \checkmark \\
Exponential ($\beta = 0.5$) & 0.956 & 1.878 & 1.000 & \checkmark & \checkmark & \checkmark \\
Exponential ($\beta = 1.0$) & 0.840 & 1.628 & 1.000 & \checkmark & \checkmark & \checkmark \\
Counterexample ($p=1/3$ at $d=1$) & 0.918 & 2.089 & N/A & \checkmark & \checkmark & $\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Non-binary branching:} Theorem A generalizes to $\Sbar(d) \leq \log_2 b$, and Theorem B* generalizes to $\rtilde(d)\le 1+\log_2 b/S(d)$ where $S(d)>0$.
...
\subsection{Interpretation of the Corrected Growth Bound}

The corrected growth theorem (Theorem~B*) shows that short-scale amplification
is controlled by the current entropy scale itself:
\[
\rtilde(d)\le 1+\frac{1}{S(d)}.
\]
When $S(d)$ is moderate or large, one-step amplification is close to 1; when
$S(d)$ is very small, larger ratios are possible.
...
possible additive jump is bounded by one bit in the binary case, preventing
unbounded one-step entropy creation.
...
\begin{enumerate}
    \item \textbf{Four fundamental theorems:} Entropic Ceiling, corrected Growth Bound (B*), Asymptotic Convergence (C*), and Geometric Bound---all proven from first principles.
...
\subsection{Ratio-Identity Lemma (Used in Theorems B* and C*)}

\begin{lemma}[Growth Ratio Decomposition]
For any depth with $S(d)>0$:
\begin{equation}
    \rtilde(d)=1+\frac{\Delta(d+1)}{S(d)}, \qquad \Delta(d+1):=S(d+1)-S(d).
\end{equation}
\end{lemma}
...
\noindent\textbf{Reproducibility note.} The corrected v3 validation scripts are:
\begin{itemize}[nosep]
    \item \texttt{/Users/stevenreid/Documents/New project/tools/rge\_math\_audit.py}
    \item \texttt{/Users/stevenreid/Documents/New project/tools/rec\_rge\_corrected\_validation.py}
\end{itemize}
...
\begin{lstlisting}[style=python, caption={Minimal corrected validation core (v3)}]
def corrected_growth_bound(S_d: float, delta_next: float) -> float:
    # Binary recursive split: 0 <= delta_next <= 1 and S_d > 0.
    return 1.0 + delta_next / S_d

def theorem_B_star_holds(S_d: float, delta_next: float) -> bool:
    # Sharp bound used in corrected theorem:
    # r_tilde(d) <= 1 + 1/S(d)
    if S_d <= 0:
        return True
    r_tilde = corrected_growth_bound(S_d, delta_next)
    return r_tilde <= 1.0 + 1.0 / S_d + 1e-12
\end{lstlisting}
