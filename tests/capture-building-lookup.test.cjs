...
test('related captures are queried with owner and full existing building identity before limiting',async()=>{
  const filters=[];
  const query={where:(...args)=>{filters.push(args);return query;},limit:n=>{assert.equal(n,61);return query;},get:async()=>({docs:[]})};
  ...
  assert.deepEqual(filters,[['ownerUid','==','owner'],['building.worldId','==','earth'],['building.sourceBuildingId','==','overture:building']]);
  assert.deepEqual(response.body,{captures:[],truncated:false,buildingScoped:true});
});
