...
test('generalized core budgeting keeps an engineered road with its mapped connector', () => {
  const previousMobileDetector = appCtx.isLikelyMobileDevice;
  const previousSurfaceProfile = appCtx.worldSurfaceProfile;
  try {
    ...
    initWorldBudgets({
      getPerfModeValue: () => 'rdt',
      limitNodesByDistance: (values, limit) => values.slice(0, limit),
      limitWaysByDistance: (values, _nodes, limit, compareFn) =>
        [...values].sort(compareFn || (() => 0)).slice(0, limit),
      nodeDistanceSq: () => 0
    });
    const nodes = {
      1: { type: 'node', id: 1, lat: 39, lon: -76 },
      2: { type: 'node', id: 2, lat: 39.001, lon: -76 },
      3: { type: 'node', id: 3, lat: 39.002, lon: -76 },
      4: { type: 'node', id: 4, lat: 39.1, lon: -76.1 },
      5: { type: 'node', id: 5, lat: 39.101, lon: -76.1 }
    };
    const bridge = {
      type: 'way', id: 'bridge', nodes: [1, 2],
      tags: { highway: 'motorway_link', bridge: 'yes', _sourceCompleteness: 'generalized' }
    };
    const connector = {
      type: 'way', id: 'connector', nodes: [2, 3],
      tags: { highway: 'motorway', _sourceCompleteness: 'generalized' }
    };
    const regional = {
      type: 'way', id: 'regional', nodes: [4, 5],
      tags: { highway: 'residential', _regionalContext: 'fixed-location', _sourceCompleteness: 'generalized' }
    };
    const loadMetrics = {
      roads: {}, buildings: {}, landuse: {}, linearFeatures: {
        railway: {}, footway: {}, cycleway: {}
    ...
    const selection = prepareWorldFeatureSelections({
      data: { elements: [...Object.values(nodes), bridge, connector, regional] },
      nodes,
      loadMetrics,
      tileBudgetCfg: {
        tileDegrees: .002, roadsPerTile: 1, roadsMinPerTile: 1,
        buildingsPerTile: 1, buildingsMinPerTile: 1,
        landusePerTile: 1, landuseMinPerTile: 1,
        poiPerTile: 1, poiMinPerTile: 1
      },
      useRdtBudgeting: false,
      enableLinearFeatures: false,
      maxRoadWays: 1,
      ...
      classifyStructureSemantics: () => ({}),
      classifyWorldSurfaceProfile: () => ({ reason: 'test', signals: { normalized: {} } }),
      isDriveableHighwayTag: Boolean,
      limitNodesByTileBudget,
      limitWaysByTileBudget,
      linearFeaturePriority: () => 0,
      poiKeyFromTags: () => '',
      roadTypePriority: (type) => type === 'motorway' ? 2 : 1
    });
    assert.ok(selection.roadWays.includes(bridge));
    assert.ok(selection.roadWays.includes(connector));
    assert.equal(loadMetrics.roads.selection.coreTopologyProtected, 2);
  } finally {
    appCtx.isLikelyMobileDevice = previousMobileDetector;
    appCtx.worldSurfaceProfile = previousSurfaceProfile;
  }
