...
function createUrbanNpcVisual(THREE, definition = {}) {
  ...
  const shoe = new THREE.MeshStandardMaterial({ color: 0x171c20, roughness: .92, metalness: .03, flatShading: true });
  ...
  add(new THREE.BoxGeometry(.42, .12, .31), outfitDark, 'NPC waist layer', [0, .75, 0]);
  ...
  for (const side of [-1, 1]) {
    ...
    add(new THREE.BoxGeometry(.18, .13, .32), shoe, 'NPC shoe', [0, -.58, .07], null, leg);
    legPivots.push(leg);
  }
