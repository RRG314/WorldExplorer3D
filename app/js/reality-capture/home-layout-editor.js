...
export async function openHomeLayoutEditor({capture,save,signal,photos=[],loadPhoto,submit,inWorld=false,importPhotos,refreshPhotos,onClose}) {
  ...
  actions.before($('[data-navigation-help]'),$('[data-photos]'));const selection=$('[data-floor]').closest('.fields');selection.classList.add('plan-selection');$('[data-editor] > nav').append(selection);actions.after($('[data-setup]'));
  ...
  function fillControls(){
    const empty=!room();
    for(const el of dialog.querySelectorAll('[data-name],[data-corner],[data-split],[data-move],[data-add-corner],[data-remove-corner],[data-door],[data-remove-door],[data-inside],[data-photos],[data-room-label-preset],[data-plan-room],[data-delete-room]'))el.disabled=empty;
  ...
  function drawPlan(){
    ...
    for(const d of floor().doors){const wall=floorWalls(floor()).find(w=>w.id===d.wall);if(!wall)continue;const length=Math.hypot(wall.b.x-wall.a.x,wall.b.z-wall.a.z),t=d.offset/length;const dp=frame.to({x:wall.a.x+(wall.b.x-wall.a.x)*t,z:wall.a.z+(wall.b.z-wall.a.z)*t});add('circle',{cx:dp.x,cy:dp.z,r:d.width/2,fill:'#093040',stroke:'#ffd369','stroke-width':.08});}
  ...
  plan.addEventListener('pointerdown',e=>{
    if(busy||!layout||!['rectangle','divide','door','corner'].includes(tool))return;
