import {makeStarterLayout,normalizeLayout,assertPlayableLayout,floorWalls,roomRing,wallKey,layoutRoomDescriptor,pointInRoom} from '../../../functions/interior-layout.mjs';
...
export async function openHomeLayoutEditor({capture,save,signal,photos=[],loadPhoto,submit}) {
  ...
  <label>Floor<select data-floor></select></label>
  <div data-grid-panel><svg data-plan viewBox="0 0 600 500" aria-label="Editable home floor plan"></svg><aside>
  <label>Room<select data-room></select></label><label>Room name<input data-name maxlength="60"></label>
  ...
  <button data-photos>Add photos to this room</button></aside></div><div data-viewer hidden></div>
  ...
  async function show3D(inside=false){try{
    ...
    if(inside){const ring=roomRing(floor(),room());let center=null;for(let x=Math.min(...ring.map(p=>p.x))+.4;x<Math.max(...ring.map(p=>p.x))&&!center;x+=.3)for(let z=Math.min(...ring.map(p=>p.z))+.4;z<Math.max(...ring.map(p=>p.z));z+=.3)if(pointInRoom({x,z},ring)){center={x,z};break;}if(!center)throw Error('This room needs more clear floor space for an inside view.');viewer.setInside?.({x:center.x,y:floor().elevation+1.6,z:center.z});}
  ...
  $('[data-floor]').onchange=()=>{floorIndex=Number($('[data-floor]').value);roomIndex=cornerIndex=0;render();};$('[data-room]').onchange=()=>{roomIndex=Number($('[data-room]').value);cornerIndex=0;render();};$('[data-corner]').onchange=()=>{cornerIndex=Number($('[data-corner]').value);render();};
  ...
  $('[data-3d-mode]').onclick=()=>show3D();$('[data-inside]').onclick=()=>show3D(true);$('[data-plan-mode]').onclick=()=>{viewer?.dispose();viewer=null;scene?.dispose();scene=null;$('[data-grid-panel]').hidden=false;$('[data-viewer]').hidden=true;render();};
