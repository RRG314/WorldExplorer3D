...
export async function openHomeLayoutEditor({capture,save,signal,photos=[],loadPhoto,submit,inWorld=false,importPhotos,refreshPhotos,onClose}) {
  ...
  <p>Private interior · draw a known room, then add its photos.</p>
  ...
  <p data-status role="status" aria-live="polite">Draw your first room inside the mapped outline.</p>
  ...
  function fillControls(){
    const empty=!room();
    for(const el of dialog.querySelectorAll('[data-name],[data-corner],[data-split],[data-move],[data-add-corner],[data-remove-corner],[data-door],[data-remove-door],[data-inside],[data-photos],[data-room-label-preset]'))el.disabled=empty;
    if(empty){$('[data-floor]').replaceChildren(...layout.floors.map((f,i)=>option(f.label,i)));$('[data-room]').replaceChildren(option('Draw your first room',''));$('[data-room-measures]').textContent='Unassigned space';return;}
  ...
  function drawPlan(){
    ...
    add('text',{x:minX+.2,y:minZ+.4,fill:'#fff','font-size':h/35/planZoom,'pointer-events':'none'}).textContent='↑ N';
    add('polygon',{points:points.map(p=>`${p.x},${p.z}`).join(' '),fill:'none',stroke:'#92d0dc','stroke-width':.08});
    for(const hole of envelope.holes)add('polygon',{points:hole.map(p=>`${p.x},${p.z}`).join(' '),fill:'#071018',stroke:'#92d0dc','stroke-width':.06,'stroke-dasharray':'.2 .15','aria-label':'Courtyard · outside the building'});
    floor().rooms.forEach((r,i)=>{const ring=roomRing(floor(),r),p=add('polygon',{points:ring.map(p=>`${p.x},${p.z}`).join(' '),fill:i===roomIndex?'#466e7b':'#203e4b',stroke:'#d0dbe0','stroke-width':.055,class:'room-shape','data-room-id':r.id,tabindex:0,role:'button','aria-label':`Room ${r.label}`});p.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();p.onclick();}};p.onclick=()=>{if(!['enter','select'].includes(tool))return;roomIndex=i;cornerIndex=selectedSurface=0;render();if(tool==='enter')void show3D(true);};const center=roomInteriorPoint(ring)||ring[0];const text=add('text',{x:center.x,y:center.z,fill:'#fff','font-size':Math.max(w,h)/38,'text-anchor':'middle','pointer-events':'none'});text.textContent=r.label;});
    ...
    (room()?.vertices||[]).forEach((id,i)=>{const p=floor().vertices[id],c=add('circle',{cx:p.x,cy:p.z,r:Math.max(w,h)/30/planZoom,fill:i===cornerIndex?'#ffcf55':'#c9e9ed',stroke:'#09222d','stroke-width':.02});c.style.cursor='grab';c.onpointerdown=e=>{e.preventDefault();cornerIndex=i;svg.setPointerCapture(e.pointerId);const move=event=>{const point=new DOMPoint(event.clientX,event.clientY).matrixTransform(svg.getScreenCTM().inverse());c.setAttribute('cx',Math.round(point.x*10)/10);c.setAttribute('cy',Math.round(point.y*10)/10);};const end=event=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',end);svg.removeEventListener('pointercancel',cancel);if(svg.hasPointerCapture(event.pointerId))svg.releasePointerCapture(event.pointerId);edit(()=>floor().vertices[id]={x:Number(c.getAttribute('cx')),z:Number(c.getAttribute('cy'))});};const cancel=()=>{svg.removeEventListener('pointermove',move);svg.removeEventListener('pointerup',end);svg.removeEventListener('pointercancel',cancel);render();};svg.addEventListener('pointermove',move);svg.addEventListener('pointerup',end);svg.addEventListener('pointercancel',cancel);};});
  }
  function drawSharedHandles(){
    ...
    for(const wall of floorWalls(floor()).filter(w=>w.rooms.length===2&&w.rooms.includes(room().id))){
      ...
      for(const [k,v] of Object.entries({cx:center.x,cy:center.z,r:Math.max(.18,22/Math.max(.01,svg.getScreenCTM().a)),fill:'#65dfce',stroke:'#09222d','stroke-width':.03}))handle.setAttribute(k,v);
  ...
  plan.addEventListener('pointerdown',e=>{
    ...
    const cancel=()=>clear();
    const end=event=>{const finish=planPoint(event);clear();void edit(()=>{let id=room()?.id;if(mode==='rectangle')id=drawRectangleRoom(floor(),id,start,finish);else{const axis=Math.abs(finish.x-start.x)>Math.abs(finish.z-start.z)?'z':'x';if(!id)throw Error('Select a room to divide.');splitRoom(floor(),id,axis,(start[axis]+finish[axis])/2);}roomIndex=floor().rooms.findIndex(r=>r.id===id);cornerIndex=0;});};
  ...
  $('[data-split]').onclick=()=>edit(()=>{splitRoom(floor(),room().id,$('[data-split-axis]').value,Number($('[data-split-at]').value));});
  ...
  if(!layout){layout=makeEmptyLayout(envelope);tool='rectangle';savedState=JSON.stringify({layout,roomPhotos});}
  if(layout){render();status(room()?'Select a room to edit or enter it.':'Draw your first room inside the outline. The remaining space stays unassigned.');}
