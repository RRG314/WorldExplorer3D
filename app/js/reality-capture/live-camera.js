...
export async function openCaptureCamera({ kind, viewLabel, signal, onPhoto }) {
  ...
  dialog.innerHTML = `<header><strong>Guided photo camera</strong><button type="button" data-camera-close aria-label="Close camera">×</button></header>
    ...
    <footer><button type="button" data-camera-shutter disabled>Take photo</button><button type="button" data-camera-done>Done</button></footer>`;
  ...
  shutter.onclick = async () => {
    ...
    taking = true; shutter.disabled = true;
    try {
      ...
      const accepted = await onPhoto(new File([blob], `capture-${Date.now()}.jpg`, { type: 'image/jpeg' }));
      if (closed) return;
      ...
      status.textContent = `${saved} photo${saved === 1 ? '' : 's'} saved. Move slightly, keep shared details, then hold still. Close the camera to change the view label.`;
    } catch (error) { if (!closed) status.textContent = error.message; }
    finally { taking = false; if (!closed) shutter.disabled = false; }
  };
  return { close };
}
