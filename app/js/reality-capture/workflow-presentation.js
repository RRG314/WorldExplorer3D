import {captureBuildingKey} from '../../../functions/capture-target.mjs';
...
export function groupCaptureBuildings(captures){
  ...
  for(const capture of captures){const b=capture.building||{},key=captureBuildingKey(b);if(!groups.has(key))groups.set(key,{key,building:b,captures:[]});groups.get(key).captures.push(capture);}
