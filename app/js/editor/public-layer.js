...
import { buildOverlayFeatureObject, disposeObject3D } from './renderer.js?v=1';
...
function currentAreaKeys() {
  ...
  const ll = typeof appCtx.worldToLatLon === 'function'
...
function applyPublishedFeatures(features = []) {
  ...
  features.forEach((feature) => {
    ...
    const center = featureWorldCenter(feature);
    if (Math.hypot(center.x, center.z) > PUBLISHED_RENDER_RANGE) return;
