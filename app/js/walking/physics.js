import { ctx as defaultContext } from "../shared-context.js?v=55";
...
function isPlanetarySurface(appCtx = defaultContext) {
  return !appCtx.activeInterior && !!(appCtx.onMoon || appCtx.onMars || appCtx.activePlanetaryBodyId);
}

function activePlanetaryBodyId(appCtx = defaultContext) {
...
function createWalkingPhysicsHelpers({
  ...
  function updateWalkPhysics(dt, finiteOr) {
    ...
    const planetaryBodyId = activePlanetaryBodyId(appCtx);
    ...
    const offerParachuteForHighDrop = (surfaceState) => {
      ...
      const highDropOffer = evaluateHighDropParachuteOffer({
        environment: isPlanetarySurface(appCtx) ? 'SPACE' : 'EARTH',
    ...
    if (jumpAction > 0.05 && !state.walker.onGround && state.walker.wallJumpTimer <= 0 && !isPlanetarySurface(appCtx)) {
    ...
    const parachuteDeployed = appCtx.isUrbanParachuteDeployed?.() === true &&
      !isPlanetarySurface(appCtx) && !state.walker.onGround && state.walker.vy < 0;
    ...
    if (forward !== 0 || strafe !== 0 || liveGpsMoved || skydivingFlight) {
      ...
      const sharedBuildingCollision = !isPlanetarySurface(appCtx) && typeof appCtx.checkBuildingCollision === "function"
        ? appCtx.checkBuildingCollision
        : null;
      const checkBuildingsFallback = !isPlanetarySurface(appCtx) && !sharedBuildingCollision && (getBuildingsArray || getNearbyBuildings);
      const checkBuildBlocks = typeof appCtx.getBuildCollisionAtWorldXZ === "function";
      const checkPlanetaryObstacles = isPlanetarySurface(appCtx);
      ...
      if (!isPlanetarySurface(appCtx) && typeof appCtx.resolveUrbanActorCollision === 'function') {
