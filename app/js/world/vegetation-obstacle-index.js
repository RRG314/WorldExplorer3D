...
export function nearbyVegetationObstacles(ctx, x, z, radius = 2) {
  ...
  if (!cache || cache.features !== features) {
    ...
    for (const tree of features) {
      ...
      const scale=Math.max(.65,Number(tree.scale)||1), r=.42*scale;
      ...
      const collider={pts,minX:tree.x-r,maxX:tree.x+r,minZ:tree.z-r,maxZ:tree.z+r,minY:baseY,maxY:baseY+4.6*scale,kind:'vegetation_trunk',sourceBuildingId:tree.id||`tree:${tree.x}:${tree.z}`};
