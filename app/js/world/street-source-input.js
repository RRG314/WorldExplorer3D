...
export function streetSourceInput(appCtx, select=()=>true) {
  ...
  return {
    roads:(appCtx.roads||[]).flatMap((road,auditIndex)=>select(road)?[{
      auditIndex,pts:road.pts,width:road.width,type:road.type,tags:road.tags,isStructureConnector:road.isStructureConnector,resolvedCrossSection:road.resolvedCrossSection,
