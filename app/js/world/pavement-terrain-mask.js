...
export function createPavementTerrainMask(appCtx,keys){
  ...
  return {layout,bytes:bytes.byteLength+addresses.byteLength,syncMaterials,
    ...
    publish(key,mask){
      ...
      addresses[index]=(slot+1)%256;addresses[index+1]=Math.floor((slot+1)/256);texture.needsUpdate=true;lookup.needsUpdate=true;
    },
    dispose(){uniforms.pavementMaskEnabled.value=0;for(const [material,entry] of hooks)restore(material,entry);hooks.clear();texture.dispose();lookup.dispose();slots.clear();}
  };
}
