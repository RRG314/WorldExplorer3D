...
try {
 for(const mobile of [false,true]) {
  ...
  await page.route(`http://127.0.0.1:${server.port}/controller-fixture`,route=>route.fulfill({contentType:'text/html',body:'<!doctype html><body><div id="ctrlContent"></div><div id="tab-settings"></div></body>'}));
  ...
  const result=await page.evaluate(async mobile=>{
   ...
   return {mobile,duringLoad,afterLoadingPlacement,firstPlay,requestedGuide,afterMovement,reloading};
  },mobile);
  results.push(result);
