...
try{
 console.log('Creating isolated staging accounts');const owner=await newAccount(true);page=owner.page;
 ...
 console.log('Opening deployed account review');await page.goto(`${origin}/account/?section=review&capture=${captureId}`);
 ...
 console.log('Account review approved fixture; testing independent runtime');const published=await call(visitor.page,'/listApprovedExteriorRepresentations',lookup);assert.equal(published.representations.length,1);
 ...
 console.log('Deleting disposable owner and checking withdrawal');await deleteTestAccount(owner);
 assert.equal((await call(visitor.page,'/listApprovedExteriorRepresentations',lookup)).representations.length,0);
