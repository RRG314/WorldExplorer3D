...
try {
  ...
  const result = await page.evaluate(async () => {
    const [{ ctx }, { updateSpaceFlightPhysics }, { updateBlackHoleEncounter }] = await Promise.all([
      ...
      import('/app/js/space/runtime.js?v=26'),
  ...
  assert.deepEqual(result.inertialVelocityAfter, result.inertialVelocityBefore, 'Turning the ship rewrote inertial velocity.');
  assert.ok(result.inertialPositionAfter[0] > 0.9, JSON.stringify(result));
