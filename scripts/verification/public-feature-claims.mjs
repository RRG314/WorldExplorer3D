...
const [manifest, packageJson] = await Promise.all([
  readFile(`${root}/config/public-feature-claims-5.0.json`, 'utf8').then(JSON.parse),
...
if (manifest.targetVersion !== '5.0.0') failures.push('targetVersion must be 5.0.0');
...
const report = {
  ...
  contract: 'world-explorer-5.0-public-feature-claims-v1',
...
assert.equal(report.ok, true, requireReady ? 'Public feature claims are not 5.0 candidate-ready.' : 'Public feature claim inventory is invalid.');
