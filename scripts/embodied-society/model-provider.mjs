...
export const GEMINI_ACTION_SCHEMA={type:'object',properties:{decisionSummary:{type:'string',maxLength:240,description:'Brief stated intent and uncertainty; not an internal reasoning trace.'},action:{anyOf:Object.entries(commandFields).map(([kind,fields])=>({type:'object',properties:{kind:{type:'string',enum:[kind]},...Object.fromEntries(fields.map(field=>[field,kind==='gather'&&field==='quantity'?{type:'integer',enum:[1],description:'Gather exactly one unit per action.'}:properties[field]]))},required:['kind',...fields],additionalProperties:false}))}},required:['action','decisionSummary'],additionalProperties:false};
