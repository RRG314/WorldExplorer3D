...
Final promotion correction: Main Menu now closes active space-flight ownership before any new title launch. The space canvas keeps one auxiliary renderer for the page runtime while old scenes, geometry, textures, and render lists are disposed between flights, preventing context churn and post-transition compositor instability. The title gate now verifies Moon -> Earth -> Mars -> Space in one page, including renderer reuse and settled visual captures. Overture building reads retry one fast non-timeout batch failure before the existing Shortbread fallback; timeout failures remain bounded. The complete production-configured release gate passes after these changes, including all 30 world-matrix locations. The candidate is ready for a Hosting-only production promotion; Auth, Firestore data, and Functions remain unchanged.

Live-smoke correction: transition ownership is now enforced below the menu layer. Every space launch has a session id, and landing animation/completion callbacks are invalid once that flight exits. Direct Earth restoration also stops after any asynchronous boundary if Main Menu or a new planetary destination has superseded it. The permanent title gate includes an abandoned Earth landing followed immediately by Mars and holds the Mars assertion beyond the old landing deadline. Because headless SwiftShader intermittently clips unrelated DOM layers in screenshots, the same gate supports headed installed-Chrome execution for final visual authority. Promotion remains pending until the full release gate, Hosting-only redeploy, and live Chrome sequence all pass with this correction.
