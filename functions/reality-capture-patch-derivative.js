...
async function rectify(bytes,quad,aspect,maxSize=1024){
  ...
  const width=Math.max(32,Math.round(maxSize*Math.min(1,aspect))),height=Math.max(32,Math.round(maxSize*Math.min(1,1/aspect))),pixels=Buffer.alloc(width*height*3),h=photoHomography(quad);
...
async function createPatchGlb(capture,preview,loadPhoto){
  ...
  const pts=isRoom&&!isHome?manualRoomFootprint(preview.room):capture.building.spatialContext.footprint;
  ...
  const patches=isHome?preview.roomPhotos.flatMap(entry=>{const room=layoutRoomDescriptor(preview.layout,entry.roomId);return entry.patches.map(p=>({...p,wall:room.surfaceIds.indexOf(p.surfaceId),roomDescriptor:room}));}):preview.patches;
  // A compressed model cap does not bound decoded GPU textures. Keep the whole
  // authored home under eight million texels, independent of photograph count.
  const maxTextureSize=isHome?Math.min(1024,2**Math.floor(Math.log2(Math.sqrt(8*1024*1024/Math.max(1,patches.length))))):1024;
  for(const patch of patches){
    ...
    const room=patch.roomDescriptor||preview.room;
    const size=isRoom?manualRoomSurfaceSize(room,patch.wall):[Math.hypot(b.x-a.x,b.z-a.z),preview.heightMeters];
    ...
    json.accessors.push({bufferView:view(Buffer.from(positions.buffer),34962),componentType:5126,count:vertexCount,type:'VEC3',min:[0,1,2].map(c=>Math.min(...Array.from({length:vertexCount},(_,i)=>positions[i*3+c]))),max:[0,1,2].map(c=>Math.max(...Array.from({length:vertexCount},(_,i)=>positions[i*3+c])))},{bufferView:view(Buffer.from(uv.buffer),34962),componentType:5126,count:vertexCount,type:'VEC2'},{bufferView:view(Buffer.from(indices.buffer),34963),componentType:5123,count:indices.length,type:'SCALAR'});
